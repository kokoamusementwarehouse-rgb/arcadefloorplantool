...
export function FloorPlanEditor() {
  ...
  return <main className="app-frame"><header className="app-header"><div className="brand"><div className="brand-mark">K</div><span className="brand-copy"><span>KOKO <b>Arcade</b></span><small className="brand-credit"><span>Design by</span><i aria-hidden="true">ϟ</i><b>Barry The Flash</b></small></span></div><ProductNavigation /><div className="header-actions"><div className="layout-context" title="This MVP has one current layout per venue">{venue.name} <span>·</span> {layout.name}</div><span className={persistenceStatus === "failed" ? "saved-state saved-state--failed" : "saved-state"}>{persistenceLabel}</span><button className="header-button" onClick={() => setFullStoreView(!fullStoreView)}>{fullStoreView ? "Editor View" : "Full Store"}</button><button className="header-button" disabled={!canExport} title={exportTitle} onClick={() => window.dispatchEvent(new Event("floorplan:export"))}><Export size={15} />Export PNG</button><ThemeControl /></div></header>
